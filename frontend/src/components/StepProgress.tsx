    ...
    <div className="bg-white border border-slate-200 text-slate-800 shadow-md shadow-slate-200/50 dark:bg-slate-900/80 dark:border-slate-800 dark:text-slate-100 dark:shadow-xl rounded-2xl p-5 transition-colors">
      <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800 transition-colors">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-bold text-slate-800 dark:text-slate-100 text-xs tracking-wider uppercase">Live Optimization Pipeline Progress</h3>
        </div>
        {result && (
          <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-300 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/30">
          ...
          return (
            <div
              ...
              className={`p-3 rounded-xl border flex items-start space-x-3 transition-all ${
                isDone
                  ? 'bg-slate-50/90 border-slate-200 text-slate-800 dark:bg-slate-950/60 dark:border-slate-800/80 dark:text-slate-200'
                  : 'bg-slate-50/40 border-slate-100 text-slate-500 dark:bg-slate-950/30 dark:border-slate-800/40 dark:text-slate-400'
              }`}
            >
              {isDone ? (
                <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
              ) : isRunning ? (
                <div className="h-4 w-4 border-2 border-cyan-600 dark:border-cyan-400 border-t-transparent rounded-full animate-spin shrink-0 mt-0.5" />
              ) : (
                <Circle className="h-4 w-4 text-slate-300 dark:text-slate-600 shrink-0 mt-0.5" />
              )}
              <div className="min-w-0 flex-1">
                <div className="text-xs font-semibold text-slate-800 dark:text-slate-100 flex items-center justify-between">
                  <span>{step.title}</span>
                  <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">#{step.id}</span>
                </div>
                <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 truncate">{step.detail}</div>
              </div>
            </div>
          );
        })}
